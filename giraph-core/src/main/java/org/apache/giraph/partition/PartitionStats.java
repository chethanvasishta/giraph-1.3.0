...
public class PartitionStats implements Writable {
  ...
  public PartitionStats(int partitionId,
	      long vertexCount,
	      RangeMap<Integer, Integer> numOfIntervalVertices,
	      long finishedVertexCount,
	      long edgeCount,
	      long messagesSentCount,
	      long messageBytesSentCount,
  ...
  public void readFields(DataInput input) throws IOException {
    ...
    int rangeMapSize = input.readInt();
    this.numOfIntervalVertices = TreeRangeMap.<Integer, Integer>create();
    if(rangeMapSize>0) {
    	int start, end, count;
    	for(int i=0;i<rangeMapSize;i++) {
    		...
    		count = input.readInt();
    		this.numOfIntervalVertices.put(Range.closedOpen(start, end), count);
    	}
    }
  }
  ...
  public void write(DataOutput output) throws IOException {
    ...
    if(numOfIntervalVertices!=null && numOfIntervalVertices.asMapOfRanges().size()>0) {
    	...
    	for(Entry<Range<Integer>, Integer> entry : numOfIntervalVertices.asMapOfRanges().entrySet()) {
    		output.writeInt(entry.getKey().lowerEndpoint());
    		output.writeInt(entry.getKey().upperEndpoint());
